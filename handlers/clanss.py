...
clan_lvl = (
    "Для создания/вступления в обьединение необходимо иметь 10 уровень фабрики"
)
create_clan_text = (
    "Чтобы создать обьединение и захватывать мир с друзьями "
    "воспользуйся форматом: \n"
    "<i>/create_clan your_name</i> \n"
    "где name название будущего обьединения пример: \n"
    "<i>/create_clan Крутой_Чел</i>\n"
    "Стоимость создания обьединения 7500 очков"
)
not_in_clan = (
    "В данный момент ты не состоишь в объединении \n"
...
async def clan_def(call: types.CallbackQuery):
    ...
    if player.clan_name == "":
        return await call.message.edit_text(
            not_in_clan, parse_mode="HTML", reply_markup=back_city
        )
    ...
    await call.message.edit_text(
        _text, parse_mode="Markdown", reply_markup=back_city
    )
...
async def join(message: types.Message):
    ...
    try:
        ...
        return await message.answer(
            "Не указано название объединения для вступления"
        )
    name = name.replace('"', "")
    player = api.player(message.from_user.id)
    if player.clan_name != "":
