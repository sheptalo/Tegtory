...
async def upgrade_factory_price(call: types.CallbackQuery):
    ...
    text = (
        ...
        f'{f'{(lvl + 3) * 400} очков' if lvl < 500 else f'{lvl - 499} SC'}\n'
        f'улучшить?'
    )
    try:
        await call.message.edit_media(
            media=InputMediaPhoto(
                media=factory_image(factory.type), caption=text
            ),
...
async def upgrade_factory(call: types.CallbackQuery):
    ...
    if player.isWorking:
        return await call.answer(
            "Во время работы нельзя прокачивать фабрику", show_alert=True
        )
    ...
    if lvl < 500:
        ...
        if stolar < lvl - 499:
            return await call.answer("Недостаточно SC", show_alert=True)
        else:
