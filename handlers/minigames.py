...
async def birzha_main(message: types.Message):
    ...
    try:
        ...
        return await message.answer(
            "Неправильно указана сумма денег отправляемая на биржу"
        )
...
async def accept_fight(call: types.CallbackQuery):
    ...
    if int(data[1]) != call.from_user.id and player2.money >= int(data[2]):
        ...
        for i in range(len(check)):
            counter += (
                1
                if factory_user_1[check[i - 1]] > factory_user_1[check[i - 1]]
                else 0
            )
...
async def fight_main(message: types.Message):
    if (
        ...
        and int(message.text.split()[1]) != "null"
        and api.factory(message.from_user.id).level >= 50
    ):
        try:
            cost = cost_get(message)
            player = api.player(message.from_user.id)
        ...
        if cost < 50000:
            return await message.answer(
                "Минимальная стоимость инспекции 50.000"
            )
        ...
        keyboard = [
            [
                types.InlineKeyboardButton(
                    text="Принять",
                    callback_data=f"accept_fight:{message.from_user.id}:{cost}",
                ),
                types.InlineKeyboardButton(
                    text="Отменить",
                    callback_data=f"no_fight:{message.from_user.id}:{cost}",
                ),
            ]
        ]
        ...
        player.money -= cost
        await message.answer(
            f"Владелец фабрики предложил провести инспекцию стоимостью в {cost:,},"
            f" Тот кто не получит рейтинг от инспектора оплачивает ее",
            reply_markup=markup,
        )
    else:
        await message.answer(
            "Добро пожаловать в центр инспекции.\n\n"
