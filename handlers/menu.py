...
⚔️ *Инспекция:*
Инспектор сравнит твою фабрику с фабрикой другого игрока и добавит тебе рейтинг
...
async def menu_cmd(message: types.Message):
    await message.answer(
        f"Привет {message.from_user.first_name}.", reply_markup=menu_reply
    )
...
async def mini_games_menu(message: types.Message):
    await message.answer(
        mini_games_text, reply_markup=mini_game_markup, parse_mode="Markdown"
    )
...
async def guide(message: types.Message):
    await message.answer(
        "Гайд со всем необходимым: https://telegra.ph/Obuchenie-Tegtory-08-30"
    )
...
async def give_money(message: types.Message):
    try:
        ...
        return await message.answer(
            "Принцип передачи денег: передать @username 1203"
        )
    ...
    await bot.send_message(
        player2.telegram_id, f"Вам передали {int(_money):,}"
    )
    await message.answer("Успешно")
    logging.info(f"кому {_id} {_money} от {message.from_user.username}")
