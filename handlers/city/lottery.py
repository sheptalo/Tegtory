...
async def buy_bronze_ticket(call: CallbackQuery):
    ...
    if player.money >= 5000:
        ...
        await call.message.edit_text(
            f"Куплен бронзовый билет с номером {new_ticket}",
            reply_markup=lottery_back_markup,
        )
    else:
        await call.message.edit_text(
            "Не хватает очков", reply_markup=lottery_back_markup
        )
...
async def buy_serebro_ticket(call: CallbackQuery):
    ...
    if player.money >= 100000:
        ...
        await call.message.edit_text(
            f"Куплен серебряный билет с номером {new_ticket}",
            reply_markup=lottery_back_markup,
        )
    else:
        await call.message.edit_text(
...
async def buy_gold_ticket(call: CallbackQuery):
    ...
    if player.money >= 10000000:
        ...
        player.tickets += f" {new_ticket}"
        await call.message.edit_text(
            f"Куплен золотой билет с номером {new_ticket}",
            reply_markup=lottery_back_markup,
        )
    else:
        await call.message.edit_text(
...
async def buy_stolar_ticket(call: CallbackQuery):
    ...
    if player.stolar >= 10:
        ...
        player.tickets += f" {new_ticket}"
        await call.message.edit_text(
            f"Куплен столар билет с номером {new_ticket}",
            reply_markup=lottery_back_markup,
        )
    else:
        await call.message.edit_text(
            "Не хватает SC", reply_markup=lottery_back_markup
        )
