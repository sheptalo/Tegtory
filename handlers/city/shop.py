...
shop_text = (
    "🎁🔥 Добро пожаловать в магазин! Здесь вы можете "
...
async def back_shop(call: CallbackQuery, state: FSMContext):
    await state.clear()
    await call.message.edit_text(shop_text, reply_markup=shop_reply)
...
async def shop_main(call: types.CallbackQuery):
    ...
    if call.message.chat.type != "private":
        text += (
            "🟥При покупке товаров связанных с фабрикой они будут *покупаться для фабрики группы*, "
            "для покупки товаров для *своей фабрики* покупайте в личных сообщениях с ботом!!🟥"
        )
    await call.message.edit_text(
        shop_text + text, reply_markup=shop_reply, parse_mode="Markdown"
    )
