...
async def buy_title_call(call: CallbackQuery):
    ...
    if title == "Богач":
        if player.money < 1000000:
            return await call.message.edit_text(
                not_enough_points, reply_markup=title_error_markup
            )

        if "Богач" in player.titles.split():
            return await call.message.edit_text(
                have_title, reply_markup=title_error_markup
            )
        ...
        if factory.lvl < 100:
            return await call.message.edit_text(
                "недостаточный уровень", reply_markup=title_error_markup
            )
        ...
        title == "Один_из_лучших"
        and Leaderboard().Money().me(api.player(call.from_user.id).iternal_id)
        <= 3
    ):
        if "Один_из_лучших" in player.titles.split():
            return call.message.edit_text(
                "У вас уже есть этот титул", reply_markup=title_error_markup
            )
    ...
    await call.message.edit_text(
        f"Куплен титул *{player.titles.split()[-1]}* ",
        reply_markup=title_error_markup,
    )
