...
async def factory_main(message: types.Message):
    ...
    if not factory.exist:
        return await message.reply(
            "у тебя еще нет фабрики", reply_markup=create_factory_markup
        )
    _type = factory_image(factory.type)
    await message.answer_photo(
        _type, caption=str(factory), reply_markup=factory_reply
    )
...
async def factory_working(call: types.CallbackQuery):
    ...
    if call.message.chat.type == "private":
        ...
        await call.message.answer(
            "Склад фабрики каждого пользователя пополнится в течении пары минут."
        )
        ...
        for member in chat_members:
            try:
                ...
                await bot.send_message(
                    member,
                    f"рабочие в чате закончили работу и ваш склад пополнился на {created:,}",
                )
            except:
                pass
        await call.message.answer(
            f"Рабочие на фабрике закончили работать, в общем произведено "
            f"{created_all:,}"
        )
...
async def start_factory(call: types.CallbackQuery):
    ...
    factory = api.factory(call.message.chat.id)
    state, tax, last_click, workers = factory.get(
        "state,tax,started_work_at,workers"
    )
    _time = 1800

    if workers == 0:
        return await call.answer(
            "Фабрику нельзя запустить если рабочих нет", show_alert=True
        )
    ...
    if not last_click or current_time - last_click >= _time:
        ...
        if tax > 20000:
            return await call.message.edit_text(
                f"У вас Неуплата налогов оплатите {tax} чтобы запустить фабрику",
                reply_markup=tax_markup,
            )

        await call.answer("Рабочие приступили к работе", show_alert=True)

        factory.set({"started_work_at": current_time, "state": 1})
        await back_factory(call)
        _ = asyncio.create_task(work_timer(call))
    else:
        await call.answer(
            f"Фабрике осталось работать {last_click + _time - current_time} секунд",
            show_alert=True,
        )
